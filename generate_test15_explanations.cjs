...
// Also remove asterisks from the text questions if they are still there
content = content.replace(/\*\*(Katherine Mansfield's adult years)\*\*/g, '$1');
content = content.replace(/\*\*\\{([0-9]+)\\}\*\*/g, '{$1}'); // This handles all numbered variables safely

// The Voyager I Space Probe
content = content.replace(/\*\*(The Voyager I Space Probe)\*\*/g, '$1');


// Update explanations object
content = content.replace(/export const test15Explanations: Record<number, string> = \{\};/g, `export const test15Explanations: Record<number, any> = ${JSON.stringify(test15Explanations, null, 2)};`);
content = content.replace(/export const test15Explanations: Record<number, any> = \{\};/g, `export const test15Explanations: Record<number, any> = ${JSON.stringify(test15Explanations, null, 2)};`);

fs.writeFileSync('src/data/test15ReadingData.ts', content);
console.log('done updating explanations');
