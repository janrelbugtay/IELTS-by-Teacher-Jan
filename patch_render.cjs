...
const regex = /const renderParagraphWithReviewHighlight = \([\s\S]*?const renderParagraphWithHighlights = \(/;

const replacement = `const renderParagraphWithReviewHighlight = (text: string, passageId: any) => {
...
if (regex.test(code)) {
  fs.writeFileSync('src/pages/ComputerReadingTest.tsx', code.replace(regex, replacement));
