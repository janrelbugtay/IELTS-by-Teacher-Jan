...
# Replace everything from export const test18Explanations to the end of the file
pattern = re.compile(r'export const test18Explanations: Record<number, string> = \{.*?\};', re.DOTALL)
if pattern.search(content):
    content = pattern.sub(new_explanations, content)
else:
    # Just in case it was already Record<number, any>
    pattern_any = re.compile(r'export const test18Explanations: Record<number, any> = \{.*?\};', re.DOTALL)
    content = pattern_any.sub(new_explanations, content)
