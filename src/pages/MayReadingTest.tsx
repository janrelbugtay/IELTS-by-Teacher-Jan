...
const passagesData = [
  {
    ...
    content: [
      "Bamboo is a remarkable plant with incredible versatility. Known as a 'wonder plant', it has been utilized for thousands of years in various cultures across the globe. Despite being a type of grass, bamboo is stronger than many types of wood, and its rapid growth rate makes it an exceptionally sustainable resource.",
      "Its applications are endless, ranging from construction materials and furniture to clothing and food. In construction, bamboo's tensile strength rivals that of steel, making it ideal for scaffolding and structural supports. Furthermore, its lightweight nature reduces transportation costs and energy consumption.",
      "Ecologically, bamboo plays a vital role in soil conservation and carbon sequestration. Its extensive root system prevents soil erosion, and a bamboo forest can absorb more carbon dioxide and release more oxygen than an equivalent stand of trees. As the world seeks greener alternatives, the wonder plant bamboo stands out as a critical element in sustainable development."
    ],
    questionBlocks: [
      {
        title: "Questions 1-5",
...
export function MayReadingTest({ submissionId, assignmentId }: { submissionId?: string, assignmentId?: string }) {
  const { user } = useAuth();
  const { id } = useParams();
