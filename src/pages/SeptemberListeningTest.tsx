...
export const SEPTEMBER_LISTENING_ANSWER_KEY: Record<number, string> = {
  1: "receptionist",
  2: "Medical",
  3: "Chastons",
  4: "appointments",
...
export function SeptemberListeningTest({ submissionId }: { submissionId?: string }) {
          ...
          <div className={`flex-1 flex flex-col justify-center px-4 py-2 font-medium text-[1em] ${isCorrect ? 'bg-white text-green-900' : 'bg-white'}`}>
            ...
            {!isCorrect && (
              <span className={`text-[0.875em] font-bold block mt-1 flex items-center gap-1 text-green-600`}>
                 <CheckCircle2 size={14} /> {SEPTEMBER_LISTENING_ANSWER_KEY[qNum]}
              </span>
            )}
              </>
            )}
    ...
    return (
      <div className="min-h-screen w-full z-50 flex flex-col items-center justify-center bg-gray-50 p-4 sm:p-8">
        <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl w-full max-w-[560px] border border-gray-100 relative overflow-y-auto max-h-full">
            <h1 className="text-2xl sm:text-3xl font-extrabold mb-2 text-center text-gray-900 tracking-tight">September IELTS Listening Test</h1>
...
<div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 1 ? 'block' : 'hidden'}`}>
    ...
    <div className="border border-black p-6 mb-8">
        <h3 className="text-center font-bold text-[20px] mb-4">Job details from employment agency</h3>
        
        <div className="grid grid-cols-[150px_1fr] gap-y-4 mb-4">
            <div className="font-bold">Role</div>
            <div><span className="font-bold mx-2">1</span><input type="text" placeholder="1" className={`ielts-input ${theme.inputBg} ${answers[1] ? 'active-state' : ''}`} value={answers[1] || ''} onChange={(e) => handleAnswerChange(1, e.target.value)} disabled={isSubmitted} /></div>
            
            <div className="font-bold">Location</div>
            <div>
                <div className="mb-2">Fordham <span className="font-bold mx-2">2</span><input type="text" placeholder="2" className={`ielts-input ${theme.inputBg} ${answers[2] ? 'active-state' : ''}`} value={answers[2] || ''} onChange={(e) => handleAnswerChange(2, e.target.value)} disabled={isSubmitted} /> Centre</div>
                <div><span className="font-bold mx-2">3</span><input type="text" placeholder="3" className={`ielts-input ${theme.inputBg} ${answers[3] ? 'active-state' : ''}`} value={answers[3] || ''} onChange={(e) => handleAnswerChange(3, e.target.value)} disabled={isSubmitted} /> Road, Fordham</div>
            </div>
            
            <div className="font-bold">Work involves</div>
            <div>
                <ul className="list-disc pl-6 mb-6 space-y-3">
                    <li>dealing with enquiries</li>
                    <li>making <span className="font-bold mx-2">4</span><input type="text" placeholder="4" className={`ielts-input ${theme.inputBg} ${answers[4] ? 'active-state' : ''}`} value={answers[4] || ''} onChange={(e) => handleAnswerChange(4, e.target.value)} disabled={isSubmitted} /> and reorganising them</li>
                    <li>maintaining the internal <span className="font-bold mx-2">5</span><input type="text" placeholder="5" className={`ielts-input ${theme.inputBg} ${answers[5] ? 'active-state' : ''}`} value={answers[5] || ''} onChange={(e) => handleAnswerChange(5, e.target.value)} disabled={isSubmitted} /></li>
            ...
            <div>
                <ul className="list-disc pl-6 mb-6 space-y-3">
                    <li><span className="font-bold mx-2">6</span><input type="text" placeholder="6" className={`ielts-input ${theme.inputBg} ${answers[6] ? 'active-state' : ''}`} value={answers[6] || ''} onChange={(e) => handleAnswerChange(6, e.target.value)} disabled={isSubmitted} /> (essential)</li>
                    <li>a calm and <span className="font-bold mx-2">7</span><input type="text" placeholder="7" className={`ielts-input ${theme.inputBg} ${answers[7] ? 'active-state' : ''}`} value={answers[7] || ''} onChange={(e) => handleAnswerChange(7, e.target.value)} disabled={isSubmitted} /> manner</li>
            ...
            <div>
                <ul className="list-disc pl-6 mb-6 space-y-3">
                    <li>a <span className="font-bold mx-2">8</span><input type="text" placeholder="8" className={`ielts-input ${theme.inputBg} ${answers[8] ? 'active-state' : ''}`} value={answers[8] || ''} onChange={(e) => handleAnswerChange(8, e.target.value)} disabled={isSubmitted} /> job – further opportunities may be available</li>
                    <li>hours: 7.45 a.m. to 9 <span className="font-bold mx-2">9</span><input type="text" placeholder="9" className={`ielts-input ${theme.inputBg} ${answers[9] ? 'active-state' : ''}`} value={answers[9] || ''} onChange={(e) => handleAnswerChange(9, e.target.value)} disabled={isSubmitted} /> p.m. Monday to Friday</li>
                    <li><span className="font-bold mx-2">10</span><input type="text" placeholder="10" className={`ielts-input ${theme.inputBg} ${answers[10] ? 'active-state' : ''}`} value={answers[10] || ''} onChange={(e) => handleAnswerChange(10, e.target.value)} disabled={isSubmitted} /> is available onsite</li>
                </ul>
            </div>
        </div>
    </div>
...
<div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 2 ? 'block' : 'hidden'}`}>
    ...
    {[
        { num: 11, q: "The museum building was originally", options: ["a factory.", "a private home.", "a hall of residence."] },
        { num: 12, q: "The university uses part of the museum building as", options: ["teaching rooms.", "a research library.", "administration offices."] },
        { num: 13, q: "What does the guide say about the entrance fee?", options: ["Visitors decide whether or not they wish to pay.", "Only children and students receive a discount.", "The museum charges extra for special exhibitions."] },
        { num: 14, q: "What are visitors advised to leave in the cloakroom?", options: ["cameras", "coats", "bags"] }
    ].map(q => (
        <div key={q.num} className="mb-6">
            <div className="flex gap-4 mb-3">
                <span className="font-bold text-[16px]">{q.num}</span>
                <span>{q.q}</span>
            </div>
    ...
    <div className="mb-4 italic text-[15px] text-gray-700">What information does the speaker give about each of the following areas of the museum?</div>
    ...
    <div className="border border-black p-4 mb-6 max-w-md mx-auto">
        <h4 className="font-bold text-center mb-4">Information</h4>
        <ul className="space-y-2">
            <li><span className="font-bold mr-4">A</span> Parents must supervise their children.</li>
            <li><span className="font-bold mr-4">B</span> There are new things to see.</li>
            <li><span className="font-bold mr-4">C</span> It is closed today.</li>
            <li><span className="font-bold mr-4">D</span> This is only for school groups.</li>
            <li><span className="font-bold mr-4">E</span> There is a quiz for visitors.</li>
            <li><span className="font-bold mr-4">F</span> It features something created by students.</li>
            <li><span className="font-bold mr-4">G</span> An expert is here today.</li>
            <li><span className="font-bold mr-4">H</span> There is a one-way system.</li>
        </ul>
    </div>

    <div className="space-y-4 mb-8">
        <h4 className="font-bold ml-12">Areas of museum</h4>
        {[
            { num: 15, text: "Four Seasons" },
            { num: 16, text: "Farmhouse Kitchen" },
            { num: 17, text: "A Year on the Farm" },
            { num: 18, text: "Wagon Walk" },
            { num: 19, text: "Bees are Magic" },
            { num: 20, text: "The Pond" }
        ].map(q => (
            <div key={q.num} className="flex items-center gap-4">
                <span className="font-bold">{q.num}</span>
                <span className="w-48">{q.text}</span>
                <input type="text" placeholder={q.num.toString()} className={`ielts-input w-12 ${answers[q.num] ? 'active-state' : ''}`} value={answers[q.num] || ''} onChange={(e) => handleAnswerChange(q.num, e.target.value.toUpperCase().slice(0, 1))} disabled={isSubmitted} />
            </div>
        ))}
    </div>
</div>
...
<div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 3 ? 'block' : 'hidden'}`}>
    ...
    <div className="mb-6 font-bold text-[15px] uppercase">Choose TWO letters, A-E.</div>
    <div className="mb-4">Which <span className="font-bold">TWO</span> educational skills were shown in the video of children doing origami?</div>
    ...
    <div className="mb-4 italic text-[15px] text-gray-700 mt-8">Questions 23-27</div>
    <div className="mb-4 italic text-[15px] text-gray-700">Which comment do the students make about each of the following children in the video?</div>
    ...
    <div className="border border-black p-4 mb-6 max-w-md mx-auto">
        <h4 className="font-bold text-center mb-4">Comments</h4>
        <ul className="space-y-2">
            <li><span className="font-bold mr-4">A</span> demonstrated independence</li>
            <li><span className="font-bold mr-4">B</span> asked for teacher support</li>
            <li><span className="font-bold mr-4">C</span> developed a competitive attitude</li>
            <li><span className="font-bold mr-4">D</span> seemed to find the activity calming</li>
            <li><span className="font-bold mr-4">E</span> seemed pleased with the results</li>
            <li><span className="font-bold mr-4">F</span> seemed confused</li>
            <li><span className="font-bold mr-4">G</span> seemed to find the activity easy</li>
        </ul>
    </div>

    <div className="space-y-4 mb-8">
        <h4 className="font-bold ml-12">Children</h4>
        {[
            { num: 23, text: "Sid" },
            { num: 24, text: "Jack" },
            { num: 25, text: "Naomi" },
    ...
    {[
        { num: 28, q: "Before starting an origami activity in class, the students think it is important for the teacher to", options: ["make models that demonstrate the different stages.", "check children understand the terminology involved.", "tell children not to worry if they find the activity difficult."] },
        { num: 29, q: "The students agree that some teachers might be unwilling to use origami in class because", options: ["they may not think that crafts are important.", "they may not have the necessary skills.", "they may worry that it will take up too much time."] },
        { num: 30, q: "Why do the students decide to use origami in their maths teaching practice?", options: ["to correct a particular misunderstanding", "to set a challenge", "to introduce a new concept"] }
    ].map(q => (
        <div key={q.num} className="mb-6">
            <div className="flex gap-4 mb-3">
                <span className="font-bold text-[16px]">{q.num}</span>
...
<div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 4 ? 'block' : 'hidden'}`}>
    ...
    <div className="border border-black p-6 mb-8">
        <h3 className="text-left font-bold text-[20px] mb-6">Victor Hugo</h3>
        
        <div className="mb-6">
            <div className="font-bold mb-2">His novel, <i>Les Misérables</i></div>
            <ul className="list-disc pl-6 mb-6 space-y-3">
                <li>It has been adapted for theatre and cinema.</li>
                <li>We know more about its overall <span className="font-bold mx-2">31</span><input type="text" placeholder="31" className={`ielts-input ${theme.inputBg} ${answers[31] ? 'active-state' : ''}`} value={answers[31] || ''} onChange={(e) => handleAnswerChange(31, e.target.value)} disabled={isSubmitted} /> than about its author.</li>
            </ul>
        </div>
        
        <div className="mb-6">
            <div className="font-bold mb-2">His early career</div>
            <ul className="list-disc pl-6 mb-6 space-y-3">
                <li>In Paris, his career was successful and he led the Romantic movement.</li>
                <li>He spoke publicly about social issues, such as <span className="font-bold mx-2">32</span><input type="text" placeholder="32" className={`ielts-input ${theme.inputBg} ${answers[32] ? 'active-state' : ''}`} value={answers[32] || ''} onChange={(e) => handleAnswerChange(32, e.target.value)} disabled={isSubmitted} /> and education.</li>
                <li>Napoleon III disliked his views and exiled him.</li>
            </ul>
        </div>

        <div className="mb-6">
            <div className="font-bold mb-2">His exile from France</div>
            <ul className="list-disc pl-6 mb-6 space-y-3">
                <li>Victor Hugo had to live elsewhere in <span className="font-bold mx-2">33</span><input type="text" placeholder="33" className={`ielts-input ${theme.inputBg} ${answers[33] ? 'active-state' : ''}`} value={answers[33] || ''} onChange={(e) => handleAnswerChange(33, e.target.value)} disabled={isSubmitted} /> .</li>
                <li>He used his income from the sale of some <span className="font-bold mx-2">34</span><input type="text" placeholder="34" className={`ielts-input ${theme.inputBg} ${answers[34] ? 'active-state' : ''}`} value={answers[34] || ''} onChange={(e) => handleAnswerChange(34, e.target.value)} disabled={isSubmitted} /> he had written to buy a house on Guernsey.</li>
            </ul>
        </div>

        <div>
            <div className="font-bold mb-2">His house on Guernsey</div>
            <ul className="list-disc pl-6 mb-6 space-y-3">
                <li>Victor Hugo lived in this house until the end of the Empire in France.</li>
                <li>The ground floor contains portraits, <span className="font-bold mx-2">35</span><input type="text" placeholder="35" className={`ielts-input ${theme.inputBg} ${answers[35] ? 'active-state' : ''}`} value={answers[35] || ''} onChange={(e) => handleAnswerChange(35, e.target.value)} disabled={isSubmitted} /> and tapestries that he valued.</li>
                <li>He bought cheap <span className="font-bold mx-2">36</span><input type="text" placeholder="36" className={`ielts-input ${theme.inputBg} ${answers[36] ? 'active-state' : ''}`} value={answers[36] || ''} onChange={(e) => handleAnswerChange(36, e.target.value)} disabled={isSubmitted} /> made of wood and turned this into beautiful wall carvings.</li>
                <li>The first floor consists of furnished areas with wallpaper and <span className="font-bold mx-2">37</span><input type="text" placeholder="37" className={`ielts-input ${theme.inputBg} ${answers[37] ? 'active-state' : ''}`} value={answers[37] || ''} onChange={(e) => handleAnswerChange(37, e.target.value)} disabled={isSubmitted} /> that have a Chinese design.</li>
                <li>The library still contains many of his favourite books.</li>
                <li>He wrote in a room at the top of the house that had a view of the <span className="font-bold mx-2">38</span><input type="text" placeholder="38" className={`ielts-input ${theme.inputBg} ${answers[38] ? 'active-state' : ''}`} value={answers[38] || ''} onChange={(e) => handleAnswerChange(38, e.target.value)} disabled={isSubmitted} /> .</li>
                <li>He entertained other writers as well as poor <span className="font-bold mx-2">39</span><input type="text" placeholder="39" className={`ielts-input ${theme.inputBg} ${answers[39] ? 'active-state' : ''}`} value={answers[39] || ''} onChange={(e) => handleAnswerChange(39, e.target.value)} disabled={isSubmitted} /> in his house.</li>
                <li>Victor Hugo's <span className="font-bold mx-2">40</span><input type="text" placeholder="40" className={`ielts-input ${theme.inputBg} ${answers[40] ? 'active-state' : ''}`} value={answers[40] || ''} onChange={(e) => handleAnswerChange(40, e.target.value)} disabled={isSubmitted} /> gave ownership of the house to the city of Paris in 1927.</li>
            </ul>
        </div>
    </div>
</div>
