...
const LoginScreen = ({ onStart, initialName, testTitle, typeLabel, isAdmin, publishConfig, onTogglePublish }: { onStart: (name: string, number: string, testMode: 'practice' | 'mock') => void, initialName: string, testTitle?: string, typeLabel?: string, isAdmin?: boolean, publishConfig?: {part1: boolean, part2: boolean}, onTogglePublish?: (part: 'part1' | 'part2') => void }) => {
...
const Header = ({ studentName, candidateNumber, timeLeft, saveStatus, onOpenSettings, onOpenSubmit, isSubmitted, testMode, isTimePaused, onTogglePause, typeLabel = 'Test' }: any) => {
    ...
    return (
        <header className="bg-gradient-to-b flex-none from-slate-700 to-slate-800 text-white px-3 md:px-5 py-2.5 flex flex-wrap md:flex-nowrap justify-between items-center shadow-lg z-20 relative gap-3 md:gap-0">
            <div className="flex items-center gap-2 md:gap-3 font-medium text-sm order-2 md:order-none">
                <div className="bg-white/10 p-1 md:p-1.5 rounded text-gray-300 hidden sm:block">
                ...
                <span className="truncate max-w-[120px] md:max-w-none">{displayName}</span>
            </div>
            
            <div className="w-full md:w-auto md:absolute md:left-1/2 md:transform md:-translate-x-1/2 flex justify-center items-center bg-black/20 px-4 py-1.5 rounded-full border border-white/10 order-1 md:order-none mb-1 md:mb-0">
                ...
                {testMode === 'practice' && typeLabel !== 'Homework' && !isSubmitted && (
                    <button 
            ...
            <div className="flex gap-2 md:gap-4 items-center order-3 md:order-none ml-auto md:ml-0">
                {!isSubmitted && (
                ...
                <button className="bg-gradient-to-b from-white to-slate-50 border border-slate-400 text-slate-800 font-semibold px-2 md:px-4 py-1 text-[12px] md:text-[13px] rounded shadow-sm hover:translate-y-[-1px] transition-transform flex items-center gap-1 md:gap-2" onClick={onOpenSettings}>
                ...
                {!isSubmitted && (
                    <button className="bg-gradient-to-b from-white to-slate-50 border border-red-200 text-red-600 font-semibold px-2 md:px-4 py-1 text-[12px] md:text-[13px] rounded shadow-sm hover:translate-y-[-1px] transition-transform flex items-center gap-2" onClick={onOpenSubmit}>
                        <span className="hidden sm:inline">{`Submit ${typeLabel}`}</span>
...
const PromptPanel = ({ activePart, textSize, testId }: any) => {
...
const FooterNavigation = ({ activePart, setActivePart, words1, words2, reviewState, toggleReview, publishConfig }: any) => {
    ...
    return (
        <footer className="bg-gradient-to-b from-slate-50 to-slate-200 border-t border-slate-300 px-6 py-3 flex-none flex justify-between items-center z-10 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex items-center gap-6">
                ...
                {(publishConfig?.part1 !== false && publishConfig?.part2 !== false) && <button 
...
export const ComputerWritingTest = ({ submissionId }: { submissionId?: string }) => {
  ...
  useEffect(() => {
    ...
    if (!state.examStarted) {
        return <LoginScreen onStart={handleStartTest} initialName={state.studentName} testTitle={testTitle} typeLabel={typeLabel} isAdmin={isAdmin} publishConfig={publishConfig} onTogglePublish={handleTogglePublish} />;
    }
