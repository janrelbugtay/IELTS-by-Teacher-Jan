          ...
          <div className="w-full max-w-[1000px] min-h-full">
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 1 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="content-box">
                      <h2 className="font-bold text-[22px] mb-6 text-center text-black">University Library Induction</h2>
                      <p className="mb-6 italic">The library serves all campuses of the University.</p>
                      
                      <div className="font-bold mb-4 text-[18px]">Student Details</div>
                      <div className="grid grid-cols-[200px_1fr] gap-4 mb-8 items-center">
                          <div>Student Name:</div>
                          <div>Jessica <span className="font-bold mx-2">1</span><input type="text" placeholder="1" className={`ielts-input ${answers[1] ? 'active-state' : ''}`} value={answers[1] || ''} onChange={(e) => handleAnswerChange(1, e.target.value)} /></div>
                          
                          <div>E-mail address (work):</div>
                          <div>jess99@ <span className="font-bold mx-2">2</span><input type="text" placeholder="2" className={`ielts-input ${answers[2] ? 'active-state' : ''}`} value={answers[2] || ''} onChange={(e) => handleAnswerChange(2, e.target.value)} /> .com</div>
                          
                          <div>Term address:</div>
                      ...
                      <table className="w-full border-collapse border border-black text-left mt-4 shadow-sm">
                          <thead>
                              <tr className="bg-gray-100">
                                  ...
                                  <th className="border border-black p-3 font-bold">Resources</th>
                                  <th className="border border-black p-3 font-bold">Borrowing rules</th>
                              </tr>
                          </thead>
                          <tbody>
                              <tr>
                                  <td className="border border-black p-3 font-bold">Books</td>
                                  <td className="border border-black p-3">textbooks</td>
                                  <td className="border border-black p-3">reference <span className="font-bold mx-2">6</span><input type="text" placeholder="6" className={`ielts-input ${answers[6] ? 'active-state' : ''}`} value={answers[6] || ''} onChange={(e) => handleAnswerChange(6, e.target.value)} /></td>
                              </tr>
                              <tr>
                                  <td className="border border-black p-3 font-bold">Journals</td>
                                  <td className="border border-black p-3">online</td>
                                  <td className="border border-black p-3">print copies in the <span className="font-bold mx-2">7</span><input type="text" placeholder="7" className={`ielts-input ${answers[7] ? 'active-state' : ''}`} value={answers[7] || ''} onChange={(e) => handleAnswerChange(7, e.target.value)} /></td>
                              </tr>
                              <tr>
                                  <td className="border border-black p-3 font-bold">Media</td>
                                  <td className="border border-black p-3">audio and video</td>
                                  <td className="border border-black p-3">documentaries and <span className="font-bold mx-2">8</span><input type="text" placeholder="8" className={`ielts-input ${answers[8] ? 'active-state' : ''}`} value={answers[8] || ''} onChange={(e) => handleAnswerChange(8, e.target.value)} /> food</td>
                              </tr>
                              <tr>
                                  <td className="border border-black p-3 font-bold">Access</td>
                                  <td className="border border-black p-3">student card or <span className="font-bold mx-2">9</span><input type="text" placeholder="9" className={`ielts-input ${answers[9] ? 'active-state' : ''}`} value={answers[9] || ''} onChange={(e) => handleAnswerChange(9, e.target.value)} /></td>
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 2 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="flex justify-center mb-8">
                      <svg viewBox="0 0 700 480" className="w-full max-w-[600px] mx-auto bg-white border-2 border-black p-4" xmlns="http://www.w3.org/2000/svg">
                          ...
                          <text x="350" y="30" textAnchor="middle" fontFamily="Arial, sans-serif" fontWeight="bold" fontSize="22" fill="black">Map of Regional Airport Terminal</text>
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 3 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="space-y-8 mb-12">
                      {[
                            { qNum: 21, text: "What happened to the earliest engineers in Germany?", options: { A: "They were less respected.", B: "They received training overseas.", C: "Their pay was very low." } },
                            { qNum: 22, text: "What was the problem with the engineer training in the 1940s?", options: { A: "Working time was too long.", B: "It took too much time.", C: "Nurses could not finish training." } },
                            { qNum: 23, text: "What was the challenge for engineers in the 1960s?", options: { A: "Work environment was terrible.", B: "Hospitals had more specialised requirements.", C: "They had limited opportunities." } },
                            { qNum: 24, text: "Why did engineers need more advanced skills?", options: { A: "Projects were complex.", B: "Patients complained.", C: "Managers requested it." } },
                            { qNum: 25, text: "What is the current situation of nursing department in the companies?", options: { A: "The number of engineers is adequate.", B: "Nurses are better trained.", C: "Hospitals can provide satisfying booking service." } }
                      ].map(q => (
                             <div key={q.qNum}>
                                <div className="font-bold mb-3 flex"><span className="w-8 shrink-0">{q.qNum}</span><span>{q.text}</span></div>
                                <div className="pl-8 space-y-1">
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 4 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="content-box mb-8">
                      <h2 className="font-bold text-[22px] mb-6 text-center text-black">History of Architecture</h2>
                      
                      <ul className="list-none space-y-5 mb-6 pl-2">
                          <li>- Building materials, structural design, and residential <span className="font-bold mx-2">31</span><input type="text" placeholder="31" className={`ielts-input ${answers[31] ? 'active-state' : ''}`} value={answers[31] || ''} onChange={(e) => handleAnswerChange(31, e.target.value)} /> are example of architectural trends.</li>
                          <li>- The professor defines architectural trends as the inability of the design aesthetics to balance or prioritize <span className="font-bold mx-2">32</span><input type="text" placeholder="32" className={`ielts-input ${answers[32] ? 'active-state' : ''}`} value={answers[32] || ''} onChange={(e) => handleAnswerChange(32, e.target.value)} /> appropriately.</li>
                          <li>- The lecturer shows the evolution of <span className="font-bold mx-2">33</span><input type="text" placeholder="33" className={`ielts-input ${answers[33] ? 'active-state' : ''}`} value={answers[33] || ''} onChange={(e) => handleAnswerChange(33, e.target.value)} /> as a architectural trends due to new construction technologies.</li>
                          <li>- Designs evolve partially in many ways, one of which is <span className="font-bold mx-2">34</span><input type="text" placeholder="34" className={`ielts-input ${answers[34] ? 'active-state' : ''}`} value={answers[34] || ''} onChange={(e) => handleAnswerChange(34, e.target.value)} /> when taller buildings are constructed.</li>
                          <li>- Architects and engineers cannot get other <span className="font-bold mx-2">35</span><input type="text" placeholder="35" className={`ielts-input ${answers[35] ? 'active-state' : ''}`} value={answers[35] || ''} onChange={(e) => handleAnswerChange(35, e.target.value)} /> to be responsible for the consequence of their actions.</li>
                      </ul>
                  </div>
                  ...
                  <div className="border-t border-gray-300 pt-8">
                      ...
                      <div className="space-y-8">
                          {[
                            { qNum: 36, text: "The speaker's story about early skyscrapers in 1910 is", options: { A: "an amusing story.", B: "relevant digression.", C: "an entertaining apology." } },
                            { qNum: 37, text: "What connection does the lecturer make between urban density and innovation?", options: { A: "like alcohol and vacations, there are fashions in public transport.", B: "as public transport becomes more convenient, more people use it.", C: "use of public transport declines as wealth increases." } },
                            { qNum: 38, text: "Construction costs was reduced in major cities from 2011 to 2014 by more than", options: { A: "10%.", B: "30%.", C: "60%." } },
                            { qNum: 39, text: "How does the lecturer evaluate steel frameworks and elevators?", options: { A: "congestion charging is less effective than road construction.", B: "they are equally ineffective.", C: "road construction is less effective than congestion charging." } },
                            { qNum: 40, text: "The lecturer thinks most planners who contribute to sprawl are", options: { A: "unconcerned.", B: "unaware.", C: "undecided." } }
                          ].map(q => (
                             <div key={q.qNum}>
                                <div className="font-bold mb-3 flex"><span className="w-8 shrink-0">{q.qNum}</span><span>{q.text}</span></div>
                                <div className="pl-8 space-y-1">
