...
export const LISTENING_ANSWER_KEY: Record<number, string> = {
    1: '3834', 2: 'HOLIDAYS/HOLIDAY', 3: 'HOME WELCOME', 4: 'FLEXIBLE', 5: 'STUDIO',
    6: '48', 7: 'WATER', 8: 'TRANSPORTATION', 9: 'DEPOSIT', 10: 'REFERENCES',
    11: 'C', 12: 'B', 13: 'B', 14: 'B', 15: 'A',
    16: 'A', 17: 'E', 18: 'B', 19: 'G', 20: 'D',
    21: 'B', 22: 'B', 23: 'B', 24: 'A', 25: 'B',
    26: 'B', 27: 'C', 28: 'F', 29: 'A', 30: 'D',
    31: 'MOUNTAINS', 32: 'STORY', 33: 'TUNNEL', 34: 'ROOF', 35: 'OUTSIDE',
    36: 'NOISE', 37: 'FIRE', 38: 'SPACE', 39: 'BASEMENT', 40: 'FLOODING'
};

export function AprilListeningTest({ submissionId }: { submissionId?: string }) {
  ...
  const submitTest = async () => {
    ...
    try {
      let title = 'IELTS Listening Test 4';
    ...
    return (
      <div className="min-h-screen w-full z-50 flex flex-col items-center justify-center bg-gray-50 p-4 sm:p-8">
        <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl w-full max-w-[560px] border border-gray-100 relative overflow-y-auto max-h-full">
            <h1 className="text-2xl sm:text-3xl font-extrabold mb-2 text-center text-gray-900 tracking-tight">April IELTS Listening Test</h1>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden text-[#333] bg-[#e1e5eb]">
      ...
      <div className="bg-white px-8 py-3 shadow-sm border-b border-gray-300 z-10 shrink-0 flex justify-between items-center">
          ...
          <div>
              <iframe src="https://drive.google.com/file/d/1jh4w6T73Xv0di0Tq1fjgEpZWFwjAvFWZ/preview" width="100%" height="150" allow="autoplay" className="border-0 rounded shadow-sm bg-white overflow-hidden max-w-[400px]"></iframe>
          </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#e6eaf2] p-6 flex justify-center items-start shadow-inner relative">
          <div className="w-full max-w-[1000px] min-h-full">
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 1 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="mb-6 font-bold text-[15px] uppercase">Write NO MORE THAN TWO WORDS AND/OR A NUMBER for each answer.</div>

                  <div className="content-box overflow-x-auto border border-gray-800 p-6 bg-white mb-6">
                      <h2 className="font-bold text-[22px] mb-6 text-center text-black">Student Accommodation</h2>
                      ...
                      <table className="w-full border-collapse border border-gray-400 text-left">
                          <thead>
                              <tr className="bg-gray-100">
                                  ...
                                  <th className="border border-gray-400 p-3">Contract</th>
                              </tr>
                          </thead>
                          <tbody>
                              <tr>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <span className="font-bold">College:</span> Single room
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <div className="mb-2">Standard: £3276 per year</div>
                                      <div>en suite: <span className="font-bold mx-1">1</span>£ <input type="text" placeholder="1" className={`ielts-input ${answers[1] ? 'active-state' : ''}`} value={answers[1] || ''} onChange={(e) => handleAnswerChange(1, e.target.value)} disabled={isSubmitted} /> per year</div>
                                      <div className="mt-2 text-sm italic">(meals are not included)</div>
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <div>36 weeks</div>
                                      <div className="mt-2">(Rooms are not available during the <span className="font-bold mx-1">2</span><input type="text" placeholder="2" className={`ielts-input ${answers[2] ? 'active-state' : ''}`} value={answers[2] || ''} onChange={(e) => handleAnswerChange(2, e.target.value)} disabled={isSubmitted} />)</div>
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <span className="font-bold mx-1">3</span><input type="text" placeholder="3" className={`ielts-input ${answers[3] ? 'active-state' : ''}`} value={answers[3] || ''} onChange={(e) => handleAnswerChange(3, e.target.value)} disabled={isSubmitted} />:<br/>Room with a family
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      £150 per week – all inclusive
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      Arrangements are <br/><span className="font-bold mx-1">4</span><input type="text" placeholder="4" className={`ielts-input ${answers[4] ? 'active-state' : ''}`} value={answers[4] || ''} onChange={(e) => handleAnswerChange(4, e.target.value)} disabled={isSubmitted} />
                                  </td>
                              </tr>
                              <tr>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <span className="font-bold">Private renting:</span><br/>
                                      Room in a shared house or a <span className="font-bold mx-1">5</span><input type="text" placeholder="5" className={`ielts-input ${answers[5] ? 'active-state' : ''}`} value={answers[5] || ''} onChange={(e) => handleAnswerChange(5, e.target.value)} disabled={isSubmitted} />
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <div>From £275 to £490 per month</div>
                                      <div className="mt-4 font-bold">Additional costs (for this year)</div>
                                      <ul className="list-disc pl-5 mt-2 space-y-2">
                                          <li>gas and electricity: approximately <span className="font-bold mx-1">6</span>£ <input type="text" placeholder="6" className={`ielts-input ${answers[6] ? 'active-state' : ''}`} value={answers[6] || ''} onChange={(e) => handleAnswerChange(6, e.target.value)} disabled={isSubmitted} /> per month</li>
                                          <li><span className="font-bold mx-1">7</span><input type="text" placeholder="7" className={`ielts-input ${answers[7] ? 'active-state' : ''}`} value={answers[7] || ''} onChange={(e) => handleAnswerChange(7, e.target.value)} disabled={isSubmitted} /> : £9</li>
                                          <li><span className="font-bold mx-1">8</span><input type="text" placeholder="8" className={`ielts-input ${answers[8] ? 'active-state' : ''}`} value={answers[8] || ''} onChange={(e) => handleAnswerChange(8, e.target.value)} disabled={isSubmitted} /></li>
                                      </ul>
                                  </td>
                                  <td className="border border-gray-400 p-3 align-top">
                                      <div>6 months</div>
                                      <div className="mt-4">Need</div>
                                      <ul className="list-disc pl-5 mt-2 space-y-2">
                                          <li><span className="font-bold mx-1">9</span><input type="text" placeholder="9" className={`ielts-input ${answers[9] ? 'active-state' : ''}`} value={answers[9] || ''} onChange={(e) => handleAnswerChange(9, e.target.value)} disabled={isSubmitted} /></li>
                                          <li>two <span className="font-bold mx-1">10</span><input type="text" placeholder="10" className={`ielts-input ${answers[10] ? 'active-state' : ''}`} value={answers[10] || ''} onChange={(e) => handleAnswerChange(10, e.target.value)} disabled={isSubmitted} /></li>
                                      </ul>
                                  </td>
                              </tr>
                          </tbody>
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 2 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="content-box mb-8 border border-gray-800 p-6 bg-white">
                      <div className="space-y-8">
                          {[
                            { qNum: 11, text: "The park was originally the site of", options: { A: "a forest.", B: "a lake.", C: "a volcano." } },
                            { qNum: 12, text: "The two chiefs met in the area to", options: { A: "trade.", B: "end the fight.", C: "celebrate their victory." } },
                            { qNum: 13, text: "The park was founded in", options: { A: "1840.", B: "1842.", C: "1942." } },
                            { qNum: 14, text: "The area used for testing is the", options: { A: "duck pond.", B: "formal gardens.", C: "iron bridge." } },
                            { qNum: 15, text: "Who provided the artwork to the village?", options: { A: "the city council", B: "local residents", C: "local museum" } },
                            { qNum: 16, text: "What is special about the Temperate House?", options: { A: "it saves a large number of endangered New Zealand plants", B: "it accommodates newly discovered plants", C: "it collects plants from around the world" } }
                          ].map(q => (
                  ...
                  <div className="border-t border-gray-300 pt-8">
                      ...
                      <div className="mb-6 font-bold text-[15px] uppercase">Write the correct letter, A-G, next to Questions 17-20.</div>

                      <div className="content-box border border-gray-800 p-6 bg-white">
                          <h2 className="font-bold text-[22px] mb-6 text-center text-black">Hobson Park</h2>
                          
                          <div className="w-full max-w-[600px] mx-auto mb-8 flex justify-center bg-white">
                              {/* Using a placeholder or image URL since image_d2c4f8.png is not guaranteed. I'll use a placeholder styled map or leave it out if we don't have the image. Actually we should just include the image tag assuming the file exists, but I'll use a blank div for safety if it doesn't. */}
                              <div className="w-full flex items-center justify-center">
                                  <img src="/hobson-park-map.png" alt="Hobson Park Map" className="w-full max-w-full h-auto object-contain border border-gray-300" referrerPolicy="no-referrer" />
                              </div>
                          </div>

                          <div className="space-y-4 max-w-[450px] mx-auto">
                              {[
                                { qNum: 17, text: "Plant nursery" },
                                { qNum: 18, text: "Pine tree hill" },
                                { qNum: 19, text: "Eagle mountain" },
                                { qNum: 20, text: "Food kiosk" }
                              ].map(q => (
                                <div key={q.qNum} className="flex items-center justify-between">
                                  <span className="font-bold w-8">{q.qNum}</span>
                                  <span className="flex-1">{q.text}</span> 
                                  <input type="text" placeholder={q.qNum.toString()} className={`ielts-input ielts-input-short uppercase ${answers[q.qNum] ? 'active-state' : ''}`} maxLength={1} value={answers[q.qNum] || ''} onChange={(e) => handleAnswerChange(q.qNum, e.target.value.toUpperCase())} disabled={isSubmitted} />
                                </div>
                              ))}
                          </div>
                      </div>
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 3 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="content-box mb-8 border border-gray-800 p-6 bg-white">
                      <div className="space-y-8">
                          {[
                            { qNum: 21, text: "What skills does Peter want to learn?", options: { A: "how to organise ideas", B: "how to handle data", C: "how to write a business proposal" } },
                            { qNum: 22, text: "What is Peter's main strength?", options: { A: "he is a good leader", B: "he can work long hours", C: "he can communicate well with many people" } },
                            { qNum: 23, text: "What will the careers officer help Peter with?", options: { A: "CV", B: "personal statement", C: "school application form" } },
                            { qNum: 24, text: "Why did Peter choose the IT business?", options: { A: "to update his knowledge in the area", B: "to be good at presentations", C: "to prepare himself for lectures" } },
                            { qNum: 25, text: "What will Peter do for the next term?", options: { A: "continue with the same language", B: "change to another language", C: "discontinue the language course" } }
                          ].map(q => (
                             <div key={q.qNum} className="mb-8">
                                <div className="font-bold mb-4 flex text-[16px]"><span className="w-8 shrink-0">{q.qNum}</span><span>{q.text}</span></div>
                                <div className="pl-8 space-y-3">
                  ...
                  <div className="border-t border-gray-300 pt-8">
                      ...
                      <div className="mb-4 italic text-[15px] text-gray-700">What job opportunities are the following companies offering?</div>
                      <div className="mb-6 font-bold text-[15px] uppercase">Choose FIVE answers from the box and write the correct letter, A-G, next to Questions 26-30.</div>

                      <div className="content-box border border-gray-800 p-6 bg-white">
                          <div className="bg-gray-100 p-6 border border-gray-300 mb-8 max-w-[600px] mx-auto">
                              <div className="font-bold mb-4 text-center border-b border-gray-300 pb-2 text-[18px]">Job opportunities</div>
                              <div className="grid grid-cols-1 gap-y-3 pl-4">
                                  <div><span className="font-bold mr-3">A</span> make business contacts</div>
                                  <div><span className="font-bold mr-3">B</span> understand customer relations</div>
                                  <div><span className="font-bold mr-3">C</span> learn time management</div>
                                  <div><span className="font-bold mr-3">D</span> use a foreign language</div>
                                  <div><span className="font-bold mr-3">E</span> travel to another country</div>
                                  <div><span className="font-bold mr-3">F</span> receive financial training</div>
                                  <div><span className="font-bold mr-3">G</span> learn project management</div>
                              </div>
                          </div>

                          <div className="font-bold mb-4 text-[18px] border-b pb-2 text-center">Companies</div>
                          <div className="space-y-4 max-w-[450px] mx-auto">
                              {[
                                { qNum: 26, text: "SAIC" },
                                { qNum: 27, text: "Gemini Technologies" },
                                { qNum: 28, text: "Deloitte" },
                                { qNum: 29, text: "Kearney" },
                                { qNum: 30, text: "Vortex" }
                              ].map(q => (
                                <div key={q.qNum} className="flex items-center justify-between">
                                  <span className="font-bold w-8">{q.qNum}</span>
              ...
              <div className={`bg-white p-10 border border-gray-300 shadow-sm text-[16px] leading-[1.8] ${currentPartIndex === 4 ? 'block' : 'hidden'}`}>
                  ...
                  <div className="content-box mb-8 border border-gray-800 p-6 bg-white">
                      <h2 className="font-bold text-[22px] mb-6 text-center text-black tracking-wide">A museum in Japan</h2>
                      
                      <ul className="list-none space-y-4 mb-8 pl-4">
                          <li>&bull; The museum accommodates a diverse range of artworks.</li>
                          <li>&bull; The museum is surrounded by <span className="font-bold mx-2">31</span><input type="text" placeholder="31" className={`ielts-input ${answers[31] ? 'active-state' : ''}`} value={answers[31] || ''} onChange={(e) => handleAnswerChange(31, e.target.value)} disabled={isSubmitted} /></li>
                          <li>&bull; The design was based on a Chinese <span className="font-bold mx-2">32</span><input type="text" placeholder="32" className={`ielts-input ${answers[32] ? 'active-state' : ''}`} value={answers[32] || ''} onChange={(e) => handleAnswerChange(32, e.target.value)} disabled={isSubmitted} /> the architect knew.</li>
                          <li>&bull; Visitors can approach the museum through a <span className="font-bold mx-2">33</span><input type="text" placeholder="33" className={`ielts-input ${answers[33] ? 'active-state' : ''}`} value={answers[33] || ''} onChange={(e) => handleAnswerChange(33, e.target.value)} disabled={isSubmitted} /> and over a bridge.</li>
                          <li>&bull; After getting into the building, a glass <span className="font-bold mx-2">34</span><input type="text" placeholder="34" className={`ielts-input ${answers[34] ? 'active-state' : ''}`} value={answers[34] || ''} onChange={(e) => handleAnswerChange(34, e.target.value)} disabled={isSubmitted} /> can be noticed immediately.</li>
                          <li>&bull; Visitors feel like being <span className="font-bold mx-2">35</span><input type="text" placeholder="35" className={`ielts-input ${answers[35] ? 'active-state' : ''}`} value={answers[35] || ''} onChange={(e) => handleAnswerChange(35, e.target.value)} disabled={isSubmitted} /> because of the light.</li>
                      </ul>

                      <div className="font-bold mb-2 text-[18px] text-blue-900 border-b border-gray-300 pb-2">Advantages of underground houses:</div>
                      <ul className="list-none space-y-4 mb-8 pl-4">
                          <li>&bull; There is less <span className="font-bold mx-2">36</span><input type="text" placeholder="36" className={`ielts-input ${answers[36] ? 'active-state' : ''}`} value={answers[36] || ''} onChange={(e) => handleAnswerChange(36, e.target.value)} disabled={isSubmitted} /></li>
                          <li>&bull; They are at a lower risk of <span className="font-bold mx-2">37</span><input type="text" placeholder="37" className={`ielts-input ${answers[37] ? 'active-state' : ''}`} value={answers[37] || ''} onChange={(e) => handleAnswerChange(37, e.target.value)} disabled={isSubmitted} /></li>
                          <li>&bull; They do better in energy conservation.</li>
                      </ul>

                      <div className="font-bold mb-2 text-[18px] text-blue-900 border-b border-gray-300 pb-2">Disadvantages of underground houses:</div>
                      <ul className="list-none space-y-4 mb-8 pl-4">
                          <li>&bull; People may feel they do not have enough <span className="font-bold mx-2">38</span><input type="text" placeholder="38" className={`ielts-input ${answers[38] ? 'active-state' : ''}`} value={answers[38] || ''} onChange={(e) => handleAnswerChange(38, e.target.value)} disabled={isSubmitted} /> (but they are already used to <span className="font-bold mx-2">39</span><input type="text" placeholder="39" className={`ielts-input ${answers[39] ? 'active-state' : ''}`} value={answers[39] || ''} onChange={(e) => handleAnswerChange(39, e.target.value)} disabled={isSubmitted} /> shopping centres).</li>
                          <li>&bull; Places suffering from <span className="font-bold mx-2">40</span><input type="text" placeholder="40" className={`ielts-input ${answers[40] ? 'active-state' : ''}`} value={answers[40] || ''} onChange={(e) => handleAnswerChange(40, e.target.value)} disabled={isSubmitted} /> cannot be used to construct underground buildings.</li>
                      </ul>
                  </div>
              </div>
