...
files.forEach(f => {
  ...
  const startIndex = content.indexOf("{block.type === 'choice' && (");
  const endIndex = content.indexOf("{block.type === 'dropdown' && (", startIndex);

  if (startIndex !== -1 && endIndex !== -1) {
    // The exact block ends right before the whitespace preceding {block.type === 'dropdown'
    const blockText = content.substring(startIndex, endIndex);
    ...
    if (lastClosing !== -1) {
      ...
      content = content.replace(fullBlockToReplace, newContent);
