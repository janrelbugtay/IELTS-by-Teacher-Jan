...
def fix(filepath, is_dashboard=False):
    ...
    if is_dashboard:
        content = re.sub(
            ...
            r"const title = getFallbackTitle(sub.assignmentId) || sub.assignmentTitle || assignment?.title || 'Unknown Test';",
            content
        )
    else:
        content = re.sub(
            r"const title = submission\.assignmentTitle \|\| getFallbackTitle\(submission\.assignmentId\) \|\| assignment\?\.title \|\| 'Test Submission';",
            r"const title = getFallbackTitle(submission.assignmentId) || submission.assignmentTitle || assignment?.title || 'Test Submission';",
            content
        )
